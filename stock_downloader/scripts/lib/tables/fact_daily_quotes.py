...
class FactDailyQuotesTable:
    ...
    def _init_table(self):
        ...
        with self.db_conn.get_connection() as conn:
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS fact_daily_quotes (
    ...
    def save(self, df: pd.DataFrame) -> int:
        ...
        for ts_code in ts_codes:
            ...
            for trade_date in dates:
                with self.db_conn.get_connection() as conn:
                    conn.execute(
                        text("DELETE FROM fact_daily_quotes WHERE ts_code = :ts_code AND trade_date = :trade_date"),
                        {"ts_code": ts_code, "trade_date": trade_date}
                    )
                    conn.commit()
        
        df.to_sql('fact_daily_quotes', self.db_conn.engine, if_exists='append', index=False)
        return len(df)
    
    def get_last_trade_date(self, ts_code: str) -> Optional[str]:
        """获取指定股票的最后交易日期"""
        query = text("SELECT MAX(trade_date) FROM fact_daily_quotes WHERE ts_code = :ts_code")
        with self.db_conn.get_connection() as conn:
    ...
    def get_history_prices(self, ts_code: str, start_date: str = None, end_date: str = None) -> pd.DataFrame:
        """获取历史行情数据"""
        query = "SELECT * FROM fact_daily_quotes WHERE ts_code = :ts_code"
