...
class IndexSyncStatusTable:
    ...
    def _init_table(self):
        ...
        with self.db_conn.get_connection() as conn:
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS index_sync_status (
    ...
    def get(self, ts_code: str) -> Optional[dict]:
        """获取指数同步状态"""
        query = text("SELECT * FROM index_sync_status WHERE ts_code = :ts_code")
        with self.db_conn.get_connection() as conn:
    ...
    def update(self, ts_code: str, last_sync_date: str, sync_status: str = 'success'):
        ...
        with self.db_conn.get_connection() as conn:
            conn.execute(text("""
                INSERT OR REPLACE INTO index_sync_status (ts_code, last_sync_date, sync_status, update_time)
    ...
    def get_all(self) -> pd.DataFrame:
        """获取所有指数的同步状态"""
        query = "SELECT * FROM index_sync_status ORDER BY ts_code"
